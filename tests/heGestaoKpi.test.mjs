...
test('extractServidores — agrega por matrícula, separa Sábado/Domingo', () => {
  const table = fakeTable([
    ['30901255', 'CARLOS HENRIQUE WANDERLEY DE OLIVEIRA', 'Domingo/Feriado', '16:00', '13:16'],
    ['30901255', 'CARLOS HENRIQUE WANDERLEY DE OLIVEIRA', 'Dia Útil/Sábado', '24:00', '06:39'],
    ['30901728', 'IGOR FRANCISCO DE OLIVEIRA COSTA', 'Dia Útil/Sábado', '24:00', '07:42']
  ]);

  const result = K.extractServidores(table);
  assert.equal(result.length, 2);

  const carlos = result.find((s) => s.matricula === '30901255');
  ...
  const igor = result.find((s) => s.matricula === '30901728');
...
test('buildDisplayNames — sem homônimo, mostra só o primeiro nome', () => {
  const servidores = [
    { nome: 'CARLOS HENRIQUE WANDERLEY DE OLIVEIRA' },
    { nome: 'IGOR FRANCISCO DE OLIVEIRA COSTA' },
    { nome: 'LUCIANO SOARES BOHNERT' }
  ];
  assert.deepEqual(K.buildDisplayNames(servidores), ['CARLOS', 'IGOR', 'LUCIANO']);
});

test('buildDisplayNames — homônimo com sobrenomes distintos ganha a inicial', () => {
  const servidores = [
    { nome: 'CARLOS HENRIQUE WANDERLEY' },
    { nome: 'CARLOS SOARES BOHNERT' }
  ];
...
test('buildRowsHTML — sem excedente resolvido ainda (fetch em segundo plano pendente), não mostra a dica', () => {
  const html = K.buildRowsHTML([servidorFixture()], ['TIAGO']);
...
test('buildRowsHTML — Domingo a 100% do autorizado COM excedente: mostra a dica e o glow, na cor roxa do bloco', () => {
  const html = K.buildRowsHTML([servidorFixture({ dom: { autMin: 16 * 60, realMin: 16 * 60, excedenteMin: 714 } })], ['TIAGO']);
...
test('buildRowsHTML — Sábado abaixo de 100% do autorizado, mesmo com excedente resolvido, não mostra a dica', () => {
  const html = K.buildRowsHTML([servidorFixture({ sab: { autMin: 24 * 60, realMin: 2 * 60 + 22, excedenteMin: 120 } })], ['TIAGO']);
...
test('buildRowsHTML — cada bloco leva data-matricula/data-bucket pra updateExcedente() localizar depois', () => {
  const html = K.buildRowsHTML([servidorFixture()], ['TIAGO']);
  assert.ok(html.includes('data-matricula="30901710"'));

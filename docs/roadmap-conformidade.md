...
- **Implementado (v0.4.0/0.4.1) → removido (v0.5.21):** o selo diário `sem autorização` marcava TODO dia com excedente sem autorização de SAEX vinculada, mesmo sabendo (ver linha acima) que esse excedente ainda **pode** virar banco por homologação ativa da chefia — não é uma perda garantida. Marcar cada dia assim soava mais alarmante do que a regra é: sem autorização prévia, o servidor ainda pode ir juntando excedente na expectativa de homologação. Removido; `authorizationScan.rowHasAuthorization()` continua existindo e sendo usado por outras partes do código. Quem responde "isso virou banco, pecúnia ou se perdeu de verdade" é a **Auditoria de Horas Perdidas** (retrospectiva, olha o resultado real do mês, não presume antes da homologação). Selo de teto legal do R5 (`> 2h/> 10h art. 4º`) continua — esse sim é uma regra objetiva, sem depender de homologação.
- **Fonte do "> autorizado" — resolvida (03/09/2026):** `GET AutorizacaoHoraExcedenteAction_execute?dataDia.asString=DD/MM/AAAA&servidor.matricula=NNN` (o backend do ícone de relógio) **é acessível pelo próprio servidor** com sessão válida — devolve a tabela de autorizações do dia (Núm., Horas Autorizadas, Tipo, limites por tipo de dia, Período). O módulo `heAuthFetch.js` consulta os dias com ícone do mês, deduplica por Núm. e classifica por bloco — hoje alimenta o KPI 4 (autorizado/feito/aberto). Falta usar a mesma fonte para o **selo diário "> autorizado"** na tabela do espelho (comparar excedente do dia × limite diário da autorização). Ver [proposta-kpis-planejamento.md §6](proposta-kpis-planejamento.md).

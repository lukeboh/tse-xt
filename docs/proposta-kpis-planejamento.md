...
> **Revisão (v0.5.16):** o campo manual (ícone ⚙, F2) foi removido — não existe
> opção pro servidor definir a própria meta de autorizado; o que o SAEX
> registrar é o que vale. `heAuthEditor.js` foi excluído; `getMatricula()` /
> `getMonthKey()` migraram pra `heAuthFetch.js`, que passa a ser a única fonte.
> O layout também mudou: cada bloco mostra **feito/autorizado** como fração à
> direita do rótulo (em vez de autorizado/feito/aberto em linhas separadas) +
> barra proporcional; o rodapé virou uma linha só, "Executado: feito /
> autorizado Autorizadas (Teto 60h/mês)" + barra. O ⚙ virou um ícone de $
> estático (não clicável), só com tooltip explicando a fonte.

Dois blocos, cada um com **Feito/Autorizado** (fração) + mini-barra proporcional:

| Bloco | Feito (fonte) | Autorizado (fonte) | Teto legal |
| :--- | :--- | :--- | :--- |
| **Semana / Sábado (+50%)** | `pecuniaWeekdaySatMinutes` (dias úteis + sábado) | SAEX (`heAuthFetch`) | 2h/dia útil · 10h/sábado · conta no teto de **60h/mês** |
| **Domingo / Feriado (+100%)** | `pecuniaSundayHolidayMinutes` (domingo + feriado) | SAEX (`heAuthFetch`) | 10h/dia, mas **pagamento vedado fora de plantão eleitoral** (art. 4º §2º Res. 22.901; art. 5º Port. 380/2026) — fora de eleição fica 00:00 |

- Sem autorizado do SAEX pra um bloco → mostra só o "feito" (sem fração/barra) nesse bloco.
- Sem autorizado do SAEX pra nenhum bloco → rodapé cai pro teto legal: "Executado: feito (Teto 60h/mês: resta HH:MM)".
...
| **F2** | Campos manuais `authWeekdaySat` / `authSundayHoliday` (⚙ no KPI 4, módulo `heAuthEditor.js`), persistidos por matrícula+mês; KPI 4 passa a mostrar **autorizado / feito / aberto** + barra + alerta de estouro. | ✅ v0.5.1 · ❌ **removido na v0.5.16** — não existe opção pro servidor definir a própria meta |
...
| **F6** | Preferências de aparência (`settings.js` + janela da extensão): preenchimento **Clássico / Gradiente** e destaque **Suave / Glow azul**, independentes, em `chrome.storage.local` (`kpiCardStyle` / `kpiCardEmphasis`), aplicadas por atributo no `<body>` e puro CSS; `chrome.storage.onChanged` sincroniza a página aberta. | ✅ v0.5.8 |
...
- Resposta — tabela **"Autorizações serviço extraordinário / Limites de horas autorizadas"**, uma linha por autorização que cobre o dia:
  ...
  | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
  | 900004 | AUTORIZAÇÃO DE PECÚNIA - PORTAL DO SERVIDOR (SAEX) | 008:54 | 29/02/2028 | Pecúnia | 002:00 | 010:00 | 000:00 | 01/08/2026 a 09/08/2026 |
  | 900002 | " | 008:00 | 29/02/2028 | Pecúnia | 000:00 | 000:00 | 010:00 | 02/08/2026 a 09/08/2026 |
...
- **Exemplo verificado — ago/2026:** Semana/Sábado autorizado **08:54** (auth 900004); Domingo/Feriado **32:00** (auths 900002 + 900003 + 179431 + 900001, 8h cada). Pecúnia feita no mês: Sáb 03:45 / Dom-Fer 24:00 → **aberto** 05:09 e 08:00.
...
- O **⚙** (F2) foi removido na v0.5.16 — o SAEX passou a ser a única fonte do autorizado, sem ajuste manual.

...
| **Em aberto** | (a) Como a extensão detecta que o mês tem HE **autorizado** — ícone/coluna de autorização (`detalharAutorizacao`), presença de pecúnia, ou consulta a outra tela? (b) É uma decisão de produto se o TSE XT deve *bloquear* o KPI "Saída p/ Zerar Mês" nesses meses ou apenas *avisar*. (c) **Falta caso de teste** para a metade "veda a **utilização**" do art. 13: na base do servidor 30000000 não há mês de 2026 com HE autorizado **e** tentativa de usufruto do banco. |
